...
int * block_table;				// pointer to block_table
int instruction_number;
int * head_pos;
int num_heads;
int num_sectors;
int num_tracks;
//pthread_mutex_t* mut_block;
int *block_suspended_list;
int root_dir_block_no;
int data_section_starting_block_no;
...
struct oft
{
	...
	char filename[20];
	int process_id;
	int block_no_of_file_metadata;
	char access_method;
	char permissions[100];
	int current_block_no_in_file_data;
	int offset_in_current_block;
	int file_descriptor;			// check how to assign
};

struct oft *head_oft, *tail_oft;

// functions to insert into oft
void insert_fcb(struct oft * fcb);
int calculate_fd();
void delete_fcb(int fd);
struct oft * return_fcb(int fd);			// will return NULL if not found
...
struct curr_dir
{
	...
	int user_id;
	int block_no;
	char pathname[1000];			// null terminate every char array used
};

struct curr_dir * curr_dir_array;

////////////
enum kind_interrupt {IO_READ,IO_WRITE};

int instruction_jump(int x, int y);
/*Linked List*/
typedef struct schedule_interrupt schedule_interrupt;
struct schedule_interrupt {
    struct schedule_interrupt* next;
    struct schedule_interrupt* prev;
    int inst_num;
    enum kind_interrupt ki;
    struct ctx_t* di;
};

struct schedule_interrupt	*headio,*tailio;

void schedule_interrupt_insert(struct schedule_interrupt* st);
void schedule_interrupt_remove() ;

struct schedule_interrupt* schedule_interrupt_return();
///////////////
...
/* Registers */
// isa_regs is a pointer to this struct
struct regs_t {
