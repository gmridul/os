...
int set_instruction_slice(int a)
{
	isa_ctx->instr_slice = a;
	printf("\nThe new instruction slice for process with uid %d is %d\n",isa_ctx->uid, isa_ctx->instr_slice);
	return a;
}
...
int handle_guest_syscalls() {
    ...
    switch (syscode) {
		...
		// add new syscall here set_instruction_slice
		// ./src/libm2skernel/context.c:56:	ctx->instr_slice
	
		case syscall_code_set_instruction_slice:
		{
			//isa_ctx->instr_slice = isa_regs->ebx;
			//retval = isa_ctx->instr_slice;
			retval = set_instruction_slice(isa_regs->ebx);
			break;
		}

		/*		402		*/
		// adding my case for read_virtual_disk
		case syscall_code_read_virtual_disk:
		{
			// here suppose we have to read from 100th block, and at offset 20
			// so we will have to skip 99 blocks, so now we will start reading from pos 99*512 + 20 bytes
			printf("\n\nIn read_virtual_disk system call, called by process having uid %d\n", isa_ctx->uid);
			long int offset, block_no, bytes_to_be_read;
			uint32_t addr;
			void * buf;
	
			// read from the isa_regs structure into these paaremeters
			// ebx,ecx, edx, esi, edi and ebp
		
			bytes_to_be_read = isa_regs->ebx;
			addr = isa_regs->ecx;
			block_no = isa_regs->edx;
			offset = isa_regs->esi;
	
			// check whether the blocks to read from belong to this user or not
			int blocks_to_be_accessed = (bytes_to_be_read + offset) / 512;
			if( (bytes_to_be_read+ offset) % 512 != 0)
			{
				blocks_to_be_accessed++;
			}
			
			int i =block_no;
			for( ;i<=block_no+blocks_to_be_accessed;i++)
			{
				if(block_table[i] == 0 || block_table[i] ==  isa_ctx->uid)
				{
				
				}
				else
				{
					printf("Cannot access blocks of some other User\n");
					exit(1);
				}
			...
			fseek(fp, 512 * (block_no - 1) + offset, SEEK_SET);		// so now cursor will be at the correct position in the file
		
			// now read bytes_to_read from the file
			buf = calloc(1, bytes_to_be_read + 1);
			//bzero(buf, bytes_to_be_read);
			long long int bytes_read = fread(buf, 1, bytes_to_be_read, fp);				// fread returns the no of bytes read
			
			printf("Bytes read from virtual disk %ld\n", bytes_read);
	
			if(ferror(fp) != 0)		// 0 is returned if no error happens
			{
				printf("Error in fread, error no: %d\n", feof(fp));
				exit(1);
			}
	
			printf("The string read from virtual disk from block no %ld at offset %ld is %s\n", block_no, offset, (char *) buf);
			
			// now we have the contents in buf, now write in memory using mem_write
			
			mem_write(isa_mem, addr, bytes_to_be_read, buf);
			schedule_interrupt* temp;
			temp = (schedule_interrupt*)malloc(sizeof(schedule_interrupt));
			temp->inst_num = instruction_number + instruction_jump(block_no,bytes_to_be_read);			// have to calculate what the instrucion_number will be
			temp->ki = IO_READ;
			temp->di = isa_ctx;
			schedule_interrupt_insert(temp);
			printf("PID:%d, In syscall_code_read_virtual_disk: Before suspending, instruction_number is %d with instruction jump being %d\n", isa_ctx->pid, instruction_number,temp->inst_num - instruction_number );
			
			ctx_set_status(isa_ctx,ctx_suspended);
			
			//printf("In syscall_code_read_virtual_disk: After suspending, instruction_number is %d\n", instruction_number);
			printf("String written in memory space\n");
			fflush(stdout);
			fclose(fp);
			free(buf);
			break;
		...
		/*		403		*/
		// adding my case for read_virtual_disk
		case syscall_code_write_virtual_disk:
		{
			// here suppose we have to write from 100th block, and at offset 20
			// so we will have to skip 99 blocks, so now we will start writing from pos 99*512 + 20 bytes
			printf("\n\nIn write_virtual_disk system call called by process having uid %d\n", isa_ctx->uid);
			long int offset, block_no, bytes_to_be_written;
			uint32_t addr;
			void * buf;
			...
			bytes_to_be_written = isa_regs->ebx;
			addr = isa_regs->ecx;
			block_no = isa_regs->edx;
			offset = isa_regs->esi;

			// check whether the blocks to write from belong to this user or not
			int blocks_to_be_accessed = (bytes_to_be_written + offset) / 512;
			if( (bytes_to_be_written+ offset) % 512 != 0)
			{
				blocks_to_be_accessed++;
			}
			...
			//printf("\nThe parameters are\n1st: %ld\n2nd: %u\n, 3rd: %ld\n, 4th: %ld\n", bytes_to_be_written, addr, block_no, offset);

			buf = calloc(1, bytes_to_be_written + 1);
			//bzero(buf, bytes_to_be_written);

			mem_read(isa_mem, addr, bytes_to_be_written, buf);
	
			printf("The string read from the memory space is %s\n", (char *) buf);
			...
			// now read bytes_to_read from the file
			long long int bytes_written = fwrite(buf, 1, bytes_to_be_written, fp);		// fread returns the no of bytes read
			
			
	
			for(i=block_no ;i<=block_no+blocks_to_be_accessed;i++)
			{
				block_table[i] = isa_ctx->uid;
			}
				
			printf("Bytes written in virtual disk at block_no %ld at offset %ld are %ld\n", block_no, offset, bytes_written);
			schedule_interrupt* temp;
			temp = (schedule_interrupt*)malloc(sizeof(schedule_interrupt));
			temp->inst_num = instruction_number + instruction_jump(block_no,bytes_to_be_written);
			temp->ki = IO_WRITE;
			temp->di = isa_ctx;
			schedule_interrupt_insert(temp);
			printf("PID: %d, In syscall_code_write_virtual_disk: Before suspending, instruction_number is %d with instruction jump being %d\n",isa_ctx->pid, instruction_number,temp->inst_num - instruction_number );
		
			ctx_set_status(isa_ctx,ctx_suspended);
			//printf("In syscall_code_write_virtual_disk: After suspending, instruction_number is %d\n", instruction_number);
			
			//ctx_set_status(isa_ctx,ctx_suspended);
			if(ferror(fp) != 0)		// 0 is returned if no error happens
			{
				printf("Error in fwrite, error no: %d\n", feof(fp));
				exit(1);
			}
...
void syscall_do() {
    int syscode = isa_regs->eax;
    int retval = 0;
    if (syscode > 325) {				// here we have added 2 syscalls for reading and writing in virtual disks with syscode 326 and 327
    ...
    } else // Pass system call to host os
    {
        ...
        switch (syscode) {
            ...
            case syscall_code_read:
            {
                ...
                fds.fd = host_fd;
                fds.events = POLLIN;
                err = poll(&fds, 1, 0);		 //it waits for one of a set of file descriptors to become ready to perform I/O
                // here polls on structure fds, hence pointer to it is passed
                // 2nd parameter is no of descriptors to wait on, since here it is 1, therefore we are passing only 1 fd structure as 1st parameter, otherwise we would have passed array as parameter
                // 3rd parameter is time to wait
                ...
                // so here we read from the user in buf (buf is char *), maybe a file, maybe from input, basically anything, and saves the read thing from the user in the memory of the GUEST OS
                if (fds.revents || (fd->flags & O_NONBLOCK)) {
                    RETVAL(read(host_fd, buf, count));			// reading from file descriptor
                    if (retval > 0) {							// retval stores the no of char read
                        mem_write(isa_mem, pbuf, retval, buf);	// writing the contents of buf in memory at address pbuf
                        syscall_debug_string("  buf", buf, count, 1);
                    }
                    
                // similar things happen for write syscall(4th one)
                    free(buf);
                    break;
                }
